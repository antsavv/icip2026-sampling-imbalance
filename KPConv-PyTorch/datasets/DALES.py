...
class DALESDataset(PointCloudDataset):
    ...
    def __init__(self, config, set='training', use_potentials=True, load_data=True):
        ...
        self.train_path = 'dales_ply'

        # List of files to process
        ply_path = join(self.path, self.train_path)

        # Get the filenames for train/validation/test datasets
        if self.set in ['training', 'validation']:
            with open(ply_path + "/train.txt") as f:
                self.all_cloud_names = f.readlines()
        elif self.set == 'test':
            with open(ply_path + "/test.txt") as f:
                self.all_cloud_names = f.readlines()

        N_files = len(self.all_cloud_names)

        # Proportion of validation scenes
        self.all_splits = [i for i in range(N_files)]

        if self.set in ['training', 'validation']:
            # self.validation_split = random.sample(self.all_splits, 6)
            # Make sure that the validation split is always the same
            self.validation_split = [0, 9, 13, 18, 22, 25]  # correspond to: 5080_54435, 5110_54475, 5135_54495,
                                                            #                5145_54470, 5165_54395, 5185_54390
        ...
        if self.set == 'training':
            ...
            raise ValueError('Unknown set for DALES data: ', self.set)
        ...
        for i in range(N_files):
            self.all_cloud_names[i] = self.all_cloud_names[i].strip()

        # List of train/validation/test files
        self.files = []
        self.cloud_names = []
        ...
        for i in range(N_files):
            if self.set == 'training':
                if self.all_splits[i] not in self.validation_split:
                    self.files += [join(ply_path + "/train/", self.all_cloud_names[i] + '.ply')]
                    self.cloud_names += [self.all_cloud_names[i]]
            elif self.set == 'validation':
                if self.all_splits[i] in self.validation_split:
                    self.files += [join(ply_path + "/train/", self.all_cloud_names[i] + '.ply')]
                    self.cloud_names += [self.all_cloud_names[i]]
            elif self.set == 'test':
                self.files += [join(ply_path + "/test/", self.all_cloud_names[i] + '.ply')]
                self.cloud_names += [self.all_cloud_names[i]]
            else:
                raise ValueError('Unknown set for DALES data: ', self.set)
    ...
    def potential_item(self, batch_i, debug_workers=False):
        ...
        while True:
            ...
            if n < 2:
                ...
                if failed_attempts > 100 * self.config.batch_num:
                    raise ValueError('It seems that dataset ' + self.all_cloud_names[cloud_ind] + ' only contains empty input spheres')
                t += [time.time()]
                t += [time.time()]
                continue
            ...
            if self.set in ['test', 'ERF']:
                ...
                input_labels = np.array([self.label_to_idx[lb] for lb in input_labels])

            t += [time.time()]

            # Data augmentation
            if self.config.do_augmentations:

                # Save an instance of the original input points for debugging purposes
                if self.config.augment_debug:
                    from plyfile import PlyData, PlyElement

                    # Define a dtype with color (structured array)
                    dt = np.dtype([('x', np.float32), ('y', np.float32), ('z', np.float32), ('red', np.uint8), ('green', np.uint8), ('blue', np.uint8)])

                    # Populate the structured array and assign green color
                    input_points_before = np.empty(input_points.shape[0], dtype=dt)
                    input_points_before['x'] = input_points[:, 0]
                    input_points_before['y'] = input_points[:, 1]
                    input_points_before['z'] = input_points[:, 2]
                    input_points_before['red'] = 0
                    input_points_before['green'] = 255
                    input_points_before['blue'] = 0

                    # Create the element and ply data, and write to file
                    element = PlyElement.describe(input_points_before, 'vertex')
                    ply_data = PlyData([element])
                    ply_data.write('input_points_before.ply')

                # Do the augmentation
                input_points, scale, R = self.augmentation_transform(input_points)

                # Save an instance of the augmented input points for debugging purposes
                if self.config.augment_debug:
                    
                    # Populate the structured array and assign green color
                    input_points_after = np.empty(input_points.shape[0], dtype=dt)
                    input_points_after['x'] = input_points[:, 0]
                    input_points_after['y'] = input_points[:, 1]
                    input_points_after['z'] = input_points[:, 2]
                    input_points_after['red'] = 255
                    input_points_after['green'] = 0
                    input_points_after['blue'] = 0

                    # Create the element and ply data, and write to file
                    element = PlyElement.describe(input_points_after, 'vertex')
                    ply_data = PlyData([element])
                    ply_data.write('input_points_after.ply')

                    # Save the files only once
                    self.config.augment_debug = False

            else:
                R = np.eye(input_points.shape[1]).astype(np.float32)
                scale = np.array([1., 1., 1.]).astype(np.float32)

            # # Color augmentation (DALES does not have color information)
            # if np.random.rand() > self.config.augment_color:
            #     input_colors *= 0

            # Additional features (except XYZ) can be added here, e.g., original height
            # input_features = np.hstack((input_colors, input_points[:, 2:] + center_point[:, 2:])).astype(np.float32)
            # input_features = (input_points[:, 2:3] + center_point[:, 2:3]).astype(np.float32)  # (num1)
            # input_features = input_points[:, 2:3].astype(np.float32)  # (num2)
            input_features = input_points.astype(np.float32)
        ...
        elif self.config.in_features_dim == 7:  # This number will change accordin to the number of additional features considered above
        ...
        input_list = self.segmentation_inputs(stacked_points, stacked_features, labels, stack_lengths)
    ...
    def load_subsampled_clouds(self):
        ...
        tree_path = []
        if self.set in ['training', 'validation']:
            tree_path = join(self.path, 'train_val_input_{:.3f}'.format(dl))
        elif self.set == 'test':
            tree_path = join(self.path, 'test_input_{:.3f}'.format(dl))
        ...
        for i, file_path in enumerate(self.files):
            ...
            if exists(KDTree_file):
                print('\nFound KDTree for cloud {:s}, sub-sampled at {:.3f}'.format(cloud_name, dl))
                ...
                # Use ply file in order not to have large numbers as in las files
                # Reflectance is not imported because it is zero
                ...
                # points = np.vstack((data.points.array['X'], data.points.array['Y'], data.points.array['Z'])).T
                # labels = data.points.array['raw_classification']

                # Subsample cloud
                sub_points, sub_labels = grid_subsampling(points.astype(np.float32),
            ...
            size = sub_labels.nbytes / 1024 / 1024
            print('{:.1f} MB loaded in {:.1f}s'.format(size, time.time() - t0))
        ...
        if self.use_potentials:
            ...
            pot_dl = self.config.in_radius / 10  # in_radius: the radius of the input sphere
            cloud_ind = 0
            ...
            for i, file_path in enumerate(self.files):
                ...
                if exists(coarse_KDTree_file):
                    ...
                    # Subsample cloud again for computational efficiency
                    # (https://github.com/HuguesTHOMAS/KPConv-PyTorch/issues/191)
        ...
        if self.set in ['validation', 'test']:
            ...
            print('\nPreparing re-projection indices for testing')

            # Get validation/test re-projection indices
            for i, file_path in enumerate(self.files):
...
class DALESSampler(Sampler):
    """Sampler for DALES"""

    def __init__(self, dataset: DALESDataset):
        ...
        if not self.dataset.use_potentials:
            ...
            for label_ind, label in enumerate(self.dataset.label_values):
                if label not in self.dataset.ignored_labels:
                    ...
                    # Select a random number amongst them
                    ...
                    if N_inds < random_pick_n:
                        ...
                        warnings.warn('When choosing random epoch indices (use_potentials=False), \
                                       class {:d}: {:s} only had {:d} available points, while we needed {:d}. \
                                       Repeating indices in the same epoch'.format(label,
    ...
    def calibration(self, dataloader, untouched_ratio=0.9, verbose=False, force_redo=False):
        ...
        if redo:
            ...
            if not breaking:
                ...
                print("ERROR: It seems that the calibration have not reached convergence. \
...
class DALESCustomBatch:
    """Custom batch definition with memory pinning for DALES"""
    ...
    def __init__(self, input_list):
        ...
        # 5 is the number of arrays (points, neighbors, pools, upsamples, lengths) per layer.
        # 7 is the number of non-layer-specific arrays (features, labels, scales, rots, cloud_inds, center_inds, input_inds), used throughout the network
    ...
    def pin_memory(self):
        """
        Manual pinning of the memory. Pinned memory (also known as page-locked memory) is a to 
        facilitate faster data transfer from the host memory (CPU) to the device memory (GPU).
        """
...
def DALESCollate(batch_data):
    return DALESCustomBatch(batch_data)
...
def debug_show_clouds(dataset, loader):
    ...
    for epoch in range(10):
        ...
        for batch_i, batch in enumerate(loader):
            ...
            print('\nCloud indices')
            print(batch.cloud_inds.dtype, batch.cloud_inds.shape)
            print('\nCenter indices')
            print(batch.center_inds.dtype, batch.center_inds.shape)
            print('\nInput indices')
            print(batch.input_inds.dtype, batch.input_inds.shape)
            ...
            print(batch.cloud_inds.is_pinned())
            print(batch.center_inds.is_pinned())
            print(batch.input_inds.is_pinned())
