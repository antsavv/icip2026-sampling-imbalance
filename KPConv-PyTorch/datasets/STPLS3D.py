...
# ----------------------------------------------------------------------------------------------------------------------
#
#      Class handling STPLS3D dataset.
...
class STPLS3DDataset(PointCloudDataset):
    ...
    def __init__(self, config, set='training', use_potentials=True, load_data=True):
        ...
        """
        PointCloudDataset.__init__(self, 'STPLS3D')
        ...
        if self.set in ['training', 'validation', 'test']:
            self.validation_split = [57]  # corresponds to: WMSC_points
        ...
        if self.set == 'training':
            ...
            raise ValueError('Unknown set for STPLS3D data: ', self.set)
        ...
        for i in range(N_files):
            if self.set == 'training':
                if self.all_splits[i] not in self.validation_split:
                    self.files += [join(ply_path + '/' + self.all_cloud_names[i] + '.ply')]
...
class STPLS3DCustomBatch:
    """Custom batch definition with memory pinning for STPLS3D"""
...
def STPLS3DCollate(batch_data):
    return STPLS3DCustomBatch(batch_data)
