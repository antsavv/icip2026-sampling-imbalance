...
class Config:
    ...
    # Choose majority class reduction ratio
    majority_ratio = 0.0
    majority_classes = []
    ...
    def load(self, path):
        ...
        for line in lines:
            ...
            if len(line_info) > 2 and line_info[0] != '#':
                ...
                elif line_info[0] == 'cls_num_list':
                    self.cls_num_list = [int(float(n)) for n in line_info[2:]]
    ...
    def save(self):
        ...
        with open(join(self.saving_path, 'parameters.txt'), "w") as text_file:
            ...
            text_file.write('\n')
            text_file.write('do_augmentations = {:d}\n'.format(self.do_augmentations))
            text_file.write('augment_debug = {:d}\n'.format(self.augment_debug))
            ...
            text_file.write('focal_loss_alpha = {:.6f}\n'.format(self.focal_loss_alpha))
            text_file.write('focal_loss_gamma = {:.6f}\n'.format(self.focal_loss_gamma))
            text_file.write('focal_loss_mode = {:s}\n'.format(self.focal_loss_mode))

            text_file.write('cls_num_list = ')
            for a in self.cls_num_list:
                text_file.write(' {:d}'.format(a))
            text_file.write('\n')
            text_file.write('ldam_loss_max_m = {:.6f}\n'.format(self.ldam_loss_max_m))
            text_file.write('ldam_loss_s = {:.6f}\n'.format(self.ldam_loss_s))

            text_file.write('ladju_tau = {:.6f}\n'.format(self.ladju_tau))
            
            text_file.write('seesaw_loss_p = {:.6f}\n'.format(self.seesaw_loss_p))
            text_file.write('seesaw_loss_q = {:.6f}\n'.format(self.seesaw_loss_q))
            text_file.write('seesaw_loss_eps = {:.6f}\n'.format(self.seesaw_loss_eps))
