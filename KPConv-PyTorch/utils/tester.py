...
#
# ----------------------------------------------------------------------------------------------------------------------
#      Class handling the test of DALES model
#  ----------------------------------------------------------------------------------------------------------------------
#
...
def display_metrics(perf_metr):
    mPerf_metr = np.mean(perf_metr)
    s = '{:5.3f} | '.format(100 * mPerf_metr)
    for metr in perf_metr:
        s += '{:5.3f} '.format(100 * metr)
...
class ModelTester:
    ...
    def cloud_segmentation_test(self, net, test_loader, config, num_votes=10):  # , debug=False
        ...
        # Initiate global prediction over test clouds
        self.test_probs = [np.zeros((lb.shape[0], nc_model)) for lb in test_loader.dataset.input_labels]

        # Test saving path
        if config.saving:
            test_path = join('test/', config.saving_path.split('/')[-2], config.saving_path.split('/')[-1])
        ...
        # If on test directly compute score
        ...
        while True:
            ...
            if last_min + 1 < new_min:
                ...
                if test_loader.dataset.set == 'test':
                    ...
                    Confs = np.zeros((len(test_loader.dataset.files), nc_tot, nc_tot), dtype=np.int32)
                    for i, file_path in enumerate(test_loader.dataset.files):
                        ...
                        # Confs
                        Confs[i, :, :] = fast_confusion(targets, preds, test_loader.dataset.label_values)
                    ...
                    IoUs = IoU_from_confusions(C)
                    PREs, RECs, F1s, _, ACC = metrics(C)

                    print('IoUs')
                    sIoU = display_metrics(IoUs)
                    print(sIoU + '\n')

                    print('Precisions')
                    sPrec = display_metrics(PREs)
                    print(sPrec + '\n')

                    print('Recalls')
                    sRec = display_metrics(RECs)
                    print(sRec + '\n')

                    print('F1-scores')
                    sF1 = display_metrics(F1s)
                    print(sF1 + '\n')
            ...
            # Break when reaching number of desired votes and save real IoU
            if last_min > num_votes:
                ...
                if test_loader.dataset.set == 'test':
                    ...
                    for i, file_path in enumerate(test_loader.dataset.files):
                        ...
                        # Extract top-2 probabilities for decision boundary analysis
                        sorted_probs = np.sort(proj_probs[i], axis=1)
                        top1_probs = sorted_probs[:, -1]  # Largest probability
                        top2_probs = sorted_probs[:, -2]  # Second largest probability
                        confidence_margin = top1_probs - top2_probs
                        
                        # Get corresponding class indices
                        sorted_indices = np.argsort(proj_probs[i], axis=1)
                        top1_class = test_loader.dataset.label_values[sorted_indices[:, -1]].astype(np.int32)
                        top2_class = test_loader.dataset.label_values[sorted_indices[:, -2]].astype(np.int32)
                        
                        # To be also used for confusion matrix calculation
                        targets = test_loader.dataset.validation_labels[i]
                        
                        # Save confidence metrics to file
                        cloud_name = file_path.split('/')[-1]
                        confidence_name = join(test_path, 'predictions', cloud_name.replace('.ply', '_confidence.txt'))
                        np.savetxt(confidence_name,
                                   np.column_stack([targets, top1_probs, top2_probs, confidence_margin,
                                                   top1_class, top2_class]),
                                   fmt='%d %.6f %.6f %.6f %d %d',
                                   header='true_label top1_prob top2_prob margin top1_class top2_class',
                                   comments='')
                        Confs[i, :, :] = fast_confusion(targets, preds, test_loader.dataset.label_values)
                    ...
                    test_file = join(test_path, 'test_perf.txt')
                    conf_file = join(test_path, 'test_conf.txt')

                    # IoUs = IoU_from_confusions(C)
                    # mIoU = np.mean(IoUs)
                    # s = '{:5.3f} | '.format(100 * mIoU)
                    # for IoU in IoUs:
                    #     s += '{:5.3f} '.format(100 * IoU)
                    
                    # print('Performance-IoU')
                    # print('-' * len(s))
                    # print(s)
                    # print('-' * len(s) + '\n')

                    # Write confusion in file
                    np.savetxt(conf_file, C, '%12d')
                    ...
                    # Write performance results in file
                    if exists(test_file):
                        with open(test_file, "a") as text_file:
                            text_file.write('IoUs: ')
                            text_file.write(sIoU + '\n')

                            text_file.write('Precisions: ')
                            text_file.write(sPrec + '\n')

                            text_file.write('Recalls: ')
                            text_file.write(sRec + '\n')

                            text_file.write('F1-scores: ')
                            text_file.write(sF1 + '\n')

                            text_file.write('Accuracy: ')
                            text_file.write('{:.3f}'.format(100*ACC) + '\n')
                    else:
                ...
                if test_loader.dataset.name=='NPM3D' or test_loader.dataset.name=='Semantic3D':
                    for i, file_path in enumerate(test_loader.dataset.files):
                        # Get the predicted labels
                        preds = test_loader.dataset.label_values[np.argmax(proj_probs[i], axis=1)].astype(np.int32)

                        # Extract cloud name and create the .txt file path
                        cloud_name = file_path.split('/')[-1].split('.')[0]  # Remove file extension
                        txt_name = join(test_path, 'predictions', cloud_name + '.txt')
                    ...
                    for i, file_path in enumerate(test_loader.dataset.files):
                        ...
                        # Get true labels
                        labels = test_loader.dataset.validation_labels[i]
                        ...
                        write_ply(test_name, [points, labels, preds], ['x', 'y', 'z', 'labels', 'preds'])
                        ...
                        # write_ply(test_name2, [points, proj_probs[i]], ['x', 'y', 'z'] + prob_names)
                        ...
                        write_ply(pot_name, [pot_points.astype(np.float32), pots], ['x', 'y', 'z', 'pots'])

                        # No need to save predictions in txt file
                        # Save ascii preds
                        # if test_loader.dataset.set == 'test':
        ...
        while True:
                ...
                for b_i, length in enumerate(lengths):
                    ...
                    if test_loader.dataset.set == 'validation':
                        ...
                        if f_ind % 20 == 0:
                            ...
                            # Also Save lbl probabilities (commented out to save disk space)
                            # probpath = join(test_path, folder, filename[:-4] + '_probs.ply')
                            # lbl_names = [test_loader.dataset.label_to_names[lb]
                            #              for lb in test_loader.dataset.label_values
                            #              if lb not in test_loader.dataset.ignored_labels]
                            # write_ply(probpath,
            ...
            if last_min + 1 < new_min:
                ...
                if test_loader.dataset.set == 'validation':
                    ...
                    PREs, RECs, F1s, _, ACC = metrics(C_tot)

                    print('IoUs')
                    sIoU = display_metrics(val_IoUs)
                    print(sIoU + '\n')
