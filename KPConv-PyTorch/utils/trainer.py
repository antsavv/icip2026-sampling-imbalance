...
class ModelTrainer:
    ...
    def train(self, net, training_loader, val_loader, config):
        ...
        if config.saving:
            ...
            if not exists(PID_file):
                with open(PID_file, "w") as file:
                    file.write('Launched with VSCode')
        ...
        for epoch in range(config.max_epoch):
            ...
            for batch in training_loader:
                ...
                loss = net.loss(outputs, batch.labels, config)
    ...
    def cloud_segmentation_validation(self, net, val_loader, config, debug=False):
        ...
        for split in val_loader.dataset.validation_split:
            if split not in val_loader.dataset.all_splits:
                return
        ...
        if not hasattr(self, 'validation_probs'):
            self.validation_probs = [np.zeros((l.shape[0], nc_model)) for l in val_loader.dataset.input_labels]
        ...
        print('{:s} mean IoU = {:.3f}%'.format(config.dataset, mIoU))
    ...
    def slam_segmentation_validation(self, net, val_loader, config, debug=True):
        ...
        for i, batch in enumerate(val_loader):
            ...
            for b_i, length in enumerate(lengths):
                ...
                # Stack all prediction for this epoch (skip frames with insufficient points)
                masked_labels = frame_labels[proj_mask]
                if masked_labels.size > 0:  # Only add if there are points
                    # Ensure arrays remain 1D even with single element
                    masked_labels = np.atleast_1d(masked_labels.ravel())
                    preds_1d = np.atleast_1d(preds.ravel())
                    predictions += [preds_1d]
