...
class SeesawLoss(nn.Module):
    ...
    def forward(self, logits, target):
        ...
        # Ensure cum_samples and target are on the same device as logits
        if self.cum_samples.device != logits.device:
            self.cum_samples = self.cum_samples.to(logits.device)
